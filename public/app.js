...
function updateReportSections() {
  ...
  el("report-section").innerHTML = '<option value="both">Both Sections</option>';
...
function generateReport() {
  ...
  currentReport.sort((a, b) => a.Registration.localeCompare(b.Registration)); renderReportTable(); el("report-summary").textContent = `${courseById(el("report-course").value)?.name || "Course"}, ${el("report-section").value === "both" ? "Both Sections" : `Section ${el("report-section").value}`}: ${currentReport.length} approved students.`; el("csv-import-panel").hidden = false;
}
