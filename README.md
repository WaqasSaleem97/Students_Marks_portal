...
- Student GitHub usernames in the enrollment list, enrollment editor, and student profile link directly to their GitHub profiles in a new tab
...
2. Grant it these project roles: **Firebase Hosting Admin** (`roles/firebasehosting.admin`), **API Keys Viewer** (`roles/serviceusage.apiKeysViewer`), **Firebase Rules Admin** (`roles/firebaserules.admin`), and **Service Usage Viewer** (`roles/serviceusage.serviceUsageViewer`). The rules role publishes the registration restrictions; Service Usage Viewer lets the Firebase CLI verify that Firestore is enabled without granting permission to enable or disable APIs. Firebase's [service-account guide](https://github.com/FirebaseExtended/action-hosting-deploy/blob/main/docs/service-account.md) lists extra roles for preview authentication domains or Cloud Run rewrites. Google's references describe the [Rules role](https://docs.cloud.google.com/iam/docs/roles-permissions/firebaserules) and the read-only [Service Usage role](https://docs.cloud.google.com/iam/docs/roles-permissions/serviceusage#service_usage_viewer).
